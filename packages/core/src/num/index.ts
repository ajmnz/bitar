...
export default (bitar: Bitar) => ({
   ...
   * ```
   */
  intl(n: number, { locale, ...options }: WithLocale<Intl.NumberFormatOptions> = {}) {
    return new Intl.NumberFormat(bitar.resolveLocale(locale), options).format(n);
  },
  ...
  currency(
    n: number,
    options: WithLocale<Intl.NumberFormatOptions> & { spaced?: boolean } = {
      spaced: true,
    }
  ) {
    let v = this.intl(n, {
      ...bitar.config.num.currency,
      ...options,
      style: "currency",
    });
  ...
  signed(
    n: number,
    options: WithLocale<Intl.NumberFormatOptions> & {
      spaced?: boolean;
      zero?: boolean;
    } = {
      spaced: false,
    ...
    let v = this.intl(n, { ...options, signDisplay: "always" });
   ...
   * ```
   */
  compact(n: number, options?: WithLocale<Intl.NumberFormatOptions>) {
    return this.intl(n, {
      notation: "compact",
   ...
   * ```
   */
  clamp(input: number, range: [number, number]): number {
    return input < range[0] ? range[0] : input > range[1] ? range[1] : input;
  },
   ...
   * ```
   */
  map(current: number, inRange: [number, number], outRange: [number, number]): number {
    return this.clamp(
   ...
   * ```
   */
  inRange(input: number, range: [number, number | null]): boolean {
    return input >= range[0] && (range[1] === null || input <= range[1]);
  },
   ...
   * ```
   */
  random(min: number, max: number, rounding: Rounding = "ceil") {
    const alg = getRoundingAlg(rounding);
    return this.clamp(alg(Math.random() * (max - min + 1) + min), [min, max]);
  },
   ...
   * ```
   */
  nearest(x: number, y: number, rounding: Rounding = "round") {
    return this.places(getRoundingAlg(rounding)(x / y) * y, getDecimalPlaces(y));
  },
   ...
   * ```
   */
  places(n: number, p: number, rounding: Rounding = "round") {
    const factor = 10 ** p;
   ...
   * distributed `n`
   */
  distribute<N extends number, G extends number>(
    n: N,
    groups: G,
    options?: {
    ...
    const base = this.places(n / groups, decimals, "floor");
